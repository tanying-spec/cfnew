# CFnew - 终端 3.0.1YUNDAN

## WeTest 历史优选

当前 Worker 自动复用 yx-auto 每 15 分钟采集的 WeTest 历史地址池：

- 创建 KV 命名空间并绑定变量名 `WETEST_HISTORY`
- 与 yx-auto 绑定同一个 KV 命名空间
- 共享键为 `wetest-ip-history-v1`，不会与 CFnew 原配置 KV 变量 `C` 冲突
- CFnew 仅负责读取，不写入历史池，也不需要配置 Cron；Cron 只保留在 yx-auto
- 共享池为空或没有绑定 KV 时，临时读取 WeTest 当前列表作为兜底，但不会写入历史
- 历史池由 yx-auto 控制为最多 500 个 IP，并优先保护 HKG/NRT IPv6
- 仅移动时 HKG 优先，仅电信时 NRT 优先
- IPv4/IPv6 同时启用时，按最终剩余名额分配约 10% IPv4 和 90% IPv6
- 所有协议与输出格式最终最多包含 50 个节点
- 最终输出会全局检查节点名称，重名节点自动追加 `-02`、`-03` 等序号
...
- 优选工具：https://github.com/tanying-spec/yx-auto
...
#### API使用
1. 使用优选项目：https://github.com/tanying-spec/yx-auto
...
[![Star History Chart](https://api.star-history.com/svg?repos=tanying-spec/cfnew&type=Timeline)](https://www.star-history.com/#tanying-spec/cfnew&Timeline&LogScale)
